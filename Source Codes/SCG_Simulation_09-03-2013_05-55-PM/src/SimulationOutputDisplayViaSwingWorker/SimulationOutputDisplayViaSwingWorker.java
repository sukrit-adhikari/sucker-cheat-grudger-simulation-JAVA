...
public class SimulationOutputDisplayViaSwingWorker extends SwingWorker<Integer, Object[]> {
    ...
    public SimulationOutputDisplayViaSwingWorker() {
        
    }

    public void startSimulationOutputDisplayViaSwingWorker(SuckerCheatGrudger_Simulation JFrame, SimulationParameters sPP){
    ...
    protected Integer doInBackground() throws Exception {
        ...
        for (Integer globalTime = 1; globalTime <= sP.getTotalIterations(); globalTime++) {
            ...
            LogWhileRunning.logEvents("Time :: "+SimulationStatistics.globalTime+" Module :: SHUFFLE POPULATION Move Around");
            ...
            if (SimulationStatistics.noOfMales > 0 && SimulationStatistics.noOfFemales > 0) {
                ...
                if (sP.getCopulationProbability() != 0) {
                    LogWhileRunning.logEvents("Time :: "+SimulationStatistics.globalTime+" Module :: COPULATION");
                    copulate(sP, SimulationStatistics.globalTime);
                }
                //System.out.println("After Copulate...");
                if (sP.getDeathFromNaturalCauseProbability() != 0) {
                    LogWhileRunning.logEvents("Time :: "+SimulationStatistics.globalTime+" Module :: RANDOM KILL Natural,Accidents,Predators");
                    killAnIndividualNaturalDeath(sP);
                }
                
                //System.out.println("Before AgeIndividuals...");
                LogWhileRunning.logEvents("Time :: "+SimulationStatistics.globalTime+" Module :: KILL INDIVIDUALS with AGE>=EXPECTANCY");
                ageIndividuals(sP);
                //System.out.println("After AgeIndividuals...");    
                
                //System.out.println("Before Infect...");
                LogWhileRunning.logEvents("Time :: "+SimulationStatistics.globalTime+" Module :: INFECT RANDOM INDIVIDUALS");
                infect(sP);
                //System.out.println("After Infect...");
                
                //System.out.println("Before askHelp");
                if(globalTime!=1){
                    LogWhileRunning.logEvents("Time :: "+SimulationStatistics.globalTime+" Module :: HELP plus KILL LOW HEALTH");
                    askHelp(sP);
                }
                //System.out.println("After AskHelp...");

            } else {
                if (SimulationStatistics.noOfFemales == 0 ) {
                    mainFrame.lbl_notifyWhySimulationHasEnded.setText("EXTINCT :: Simulation has ended as there are no Females left in the population.");
                } else if (SimulationStatistics.noOfMales == 0) {
                    mainFrame.lbl_notifyWhySimulationHasEnded.setText("EXTINCT :: Simulation has ended as there are no Males left in the population.");
                }
                mainFrame.jProgressBar_SimulationStatus.setValue(100);
                mainFrame.jProgressBar_SimulationStatus.setString("100%");
                break;
            }
            publish(new Object[]{globalTime-1+"-"+globalTime, (SimulationStatistics.noOfMales + SimulationStatistics.noOfFemales), SimulationStatistics.noOfBirths, SimulationStatistics.noOfDeaths_Natural, SimulationStatistics.noOfDeaths_Ageing, SimulationStatistics.noOfDeaths_Infection, SimulationStatistics.noOfSuckers, SimulationStatistics.noOfCheats, SimulationStatistics.noOfGrudgers, SimulationStatistics.noOfInfectedIndividuals, SimulationStatistics.noOf_SS_Genes, SimulationStatistics.noOf_CC_Genes, SimulationStatistics.noOf_GG_Genes, SimulationStatistics.noOf_SC_Genes, SimulationStatistics.noOf_SG_Genes, SimulationStatistics.noOf_CG_Genes, SimulationStatistics.noOfMales, SimulationStatistics.noOfFemales});
            SimulationStatistics.globalTime++;
        }
        
        if(SimulationStatistics.noOfFemales>0&&SimulationStatistics.noOfMales>0){
            mainFrame.lbl_notifyWhySimulationHasEnded.setText("SURVIDED :: Population survived for given iterations.");
        }
        LogWhileRunning.logEvents("SIMULATION ENDED");
//        while(LogWhileRunning.log.size()!=0){
//            mainFrame.txtArea_log.append(LogWhileRunning.log.get(0)+"\n");
//            LogWhileRunning.log.remove(0);
//            //mainFrame.txtArea_log.setCaretPosition(mainFrame.txtArea_log.getDocument().getLength());
//        }
        
        
        mainFrame.check_enableLiveLogging.setEnabled(true);
        mainFrame.btn_startSimulation.setEnabled(true);
        mainFrame.jRadioButton1.setEnabled(true);
        mainFrame.jRadioButton2.setEnabled(true);
        return 0;
    }
    ...
    private void createIndividuals(SimulationParameters sP) {
        ...
        LogWhileRunning.logEvents("Time :: "+SimulationStatistics.globalTime+" CREATE INDIVIDUALS");
        ...
        for (i = 1; i <= s; i++) { // Create suckers with pure SS genes
            ...
            LogWhileRunning.logIndividualParameters(newIndividual,"Introduced into population");
        }
    ...
    private void killAnIndividualNaturalDeath(SimulationParameters sP) {
        if (individuals.size() == 0 || individuals.isEmpty()) {
            mainFrame.lbl_notifyWhySimulationHasEnded.setText("EXTINCT :: Simulation has ended as the entire population has gone extinct.");
            return;
        }
    ...
    protected void process(List<Object[]> chunks) {
            ...
            Integer threadCount = ++SimulationStatistics.threadCount;
            
            
            Integer totalIterations = sP.getTotalIterations();
            Integer progressOfSimulation = (SimulationStatistics.globalTime * 100) / (totalIterations);
        
        try {
            if( (SimulationStatistics.noOfMales +SimulationStatistics.noOfFemales)>0&&SimulationStatistics.threadCount>0){
            ...
            SimulationStatistics.averageSuckersPercentage = ((     (SimulationStatistics.averageSuckersPercentage)* (threadCount-1)  ) + (suckersPercentage) )/(threadCount);
            SimulationStatistics.averageCheatsPercentage = ((     (SimulationStatistics.averageCheatsPercentage)* (threadCount-1)  ) + (cheatsPercentage) )/(threadCount);
            SimulationStatistics.averageGrudgersPercentage = ((     (SimulationStatistics.averageGrudgersPercentage)* (threadCount-1)  ) + (grudgersPercentage) )/(threadCount);
            ...
            if(mainFrame.jRadioButton1.isSelected()){
                ...
                mainFrame.jProgressBar_SuckersPercentage.setValue(SimulationStatistics.averageSuckersPercentage);
                mainFrame.jProgressBar_CheatsPercentage.setValue(SimulationStatistics.averageCheatsPercentage);
                mainFrame.jProgressBar_GrudgersPercentage.setValue(SimulationStatistics.averageGrudgersPercentage);
            }
        ...
        if(mainFrame.check_enableLiveLogging.isSelected()){
            ...
            while(  (!LogWhileRunning.log.isEmpty() && noOfLogsDisplay<=(1000*SimulationStatistics.globalTime)) ||((!LogWhileRunning.log.isEmpty() && progressOfSimulation>=99))  ){ // Until its Empty
             mainFrame.txtArea_log.append(LogWhileRunning.log.get(0)+"\n");
             LogWhileRunning.log.remove(0);
             noOfLogsDisplay++;
            }
        ...
        mainFrame.simulationStatisticsTable.scrollRectToVisible(mainFrame.simulationStatisticsTable.getCellRect(mainFrame.simulationStatisticsTable.getRowCount()-1, 0, true));
    }
